...
def capture_project_manifests(cli_args) -> dict:
    ...
    if cli_args.project_name:
        ...
        while True:
            ...
            if is_valid_project_name(user_input):
                ...
                print(
                    "⚠️ Invalid project name (contains illegal characters or folder exists). Please try again."
                )
    ...
    if cli_args.theme:
        ...
        while True:
            ...
            user_input = (
                input("Enter project theme (1-4 OR theme name): ").strip().lower()
            )
    ...
    if theme != "minimal" and not cli_args.skip:
        ...
        while True:
            if re.search(r"[^0-9.]", version_input) and version_input != "":
                version_input = input(
                    'Please enter a valid version number (Only contains numbers and dots/"."): '
                )
            elif version_input == "":
                break
            else:
