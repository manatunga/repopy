...
def parse_arguments() -> argparse.Namespace:
    ...
    init_parser.add_argument(
        "project_name", nargs="?", help="Name of the target project folder to create"
    )
    ...
    clone_parser = subparsers.add_parser(
        "clone", help="Clones a remote Git repository to the local machine"
    )

    # Positional argument to add remote Git URL
    clone_parser.add_argument(
        "repo_url", help="Repository URL from Github to clone locally"
    )

    # Name flag (-n / --name) to pass an optional name for the to-be-cloned repository
    clone_parser.add_argument("-n", "--name", help="Name of the project folder")
