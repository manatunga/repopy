...
def test_git_engine_path_derivation_custom_name():
    engine = GitEngine(
        "https://github.com/user/demo-repo.git", dir_name="my_custom_dir"
    )
...
def test_clone_repository_failure_exit_code(monkeypatch):
    def fake_run(cmd, *args, **kwargs):
        return subprocess.CompletedProcess(
            args=cmd, returncode=128, stdout="", stderr="Fatal error"
        )
...
def test_git_link_nothing_to_commit(monkeypatch):
    ...
    def fake_run(cmd, *args, **kwargs):
        ...
        if cmd[1] == "commit":
            return subprocess.CompletedProcess(
                args=cmd, returncode=1, stdout="nothing to commit"
            )
        return subprocess.CompletedProcess(args=cmd, returncode=0, stdout="")
...
def test_git_link_early_abort_on_failure(monkeypatch):
    ...
    def fake_run(cmd, *args, **kwargs):
        ...
        if cmd[1] == "remote":
            return subprocess.CompletedProcess(
                args=cmd, returncode=128, stdout="remote already exists"
            )
        return subprocess.CompletedProcess(args=cmd, returncode=0, stdout="")
...
def test_git_link_stage_failure(monkeypatch):
    def fake_run(cmd, *args, **kwargs):
        if cmd[1] == "add":
            return subprocess.CompletedProcess(
                args=cmd, returncode=1, stderr="stage error"
            )
...
def test_git_link_commit_real_failure(monkeypatch):
    def fake_run(cmd, *args, **kwargs):
        if cmd[1] == "commit":
            return subprocess.CompletedProcess(
                args=cmd, returncode=1, stdout="author identity unknown"
            )
        return subprocess.CompletedProcess(args=cmd, returncode=0)
...
def test_git_link_branch_failure(monkeypatch):
    def fake_run(cmd, *args, **kwargs):
        if cmd[1] == "branch":
            return subprocess.CompletedProcess(
                args=cmd, returncode=1, stderr="branch error"
            )
        return subprocess.CompletedProcess(args=cmd, returncode=0)
...
def test_git_link_push_failure(monkeypatch):
    def fake_run(cmd, *args, **kwargs):
        if cmd[1] == "push":
            return subprocess.CompletedProcess(
                args=cmd, returncode=1, stderr="remote rejected"
            )
        return subprocess.CompletedProcess(args=cmd, returncode=0)
